...
#tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-3B-Instruct",cache_dir='autodl-tmp', token=access_token)
...
#    return tokenized
def tokenize_function(samples):
    model_inputs = tokenizer(samples["text"], padding="max_length", truncation=True, max_length=256)
    labels = tokenizer(samples["labels"], padding="max_length", truncation=True, max_length=256)
...
training_args = Seq2SeqTrainingArguments(
    output_dir="autodl-tmp/mbart_german_simplified",
    per_device_train_batch_size=4,
    per_device_eval_batch_size=4,
    num_train_epochs=4,
    logging_steps=10,
...
import torch.nn.functional as F
class CustomTrainer(Seq2SeqTrainer):
    def __init__(self, *args, difficult_dict=None, tokenizer=None, gamma=1, beta=0.5, **kwargs):
        super().__init__(*args, **kwargs)
...
trainer.save_model("mbart_german_simplified_lora")
...
with open("VIBoutput1.json", "w") as f:
